...
| `/sen/[id]`              | One dream: the tile — up to five photographs in its first pick (D84), and from two up a carousel of the collage and then each photograph (D86), placed in `CollageEditor` (D85) — the shelf of its photographs, the facts with „Naposledy upraveno“ (D77), the heart, Upravit and Smazat                                                                                    |
...
it floats in, and which way the point runs under a finger (D81) — and a cell's crop, `cellFocal` (D85),
`ui/placing.ts`'s test (a drag against the finger, a pinch, a lifted finger that does not jump, D85),
`ui/carousel.test.ts` (which slide is showing and where a dot scrolls to, D86), and the D83–D84
